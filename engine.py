...
def _check_js(fn, html):
    """
    Reject a page whose inline JavaScript does not parse.

    Twice now a single-escaped \\n in the Python source became a real newline
    inside a JS string literal, silently breaking a whole page's simulation.
    Catching it at build time is the only reliable guard, because the output
    looks completely normal to every other check.
    """
    ...
    try:
        ...
        if r.returncode:
            raise SystemExit(f"\n{fn}: inline JavaScript does not parse\n{r.stderr}")
    finally:
        os.unlink(path)
